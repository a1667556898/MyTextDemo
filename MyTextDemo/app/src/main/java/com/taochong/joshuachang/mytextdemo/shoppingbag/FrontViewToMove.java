...
public class FrontViewToMove {
    ...
    private int xToMove = 200;// 视图所要被移动的距离，默认200 不同手机有不同的像素密度 多以要写
    ...
    public FrontViewToMove(View frontView, ListView listView, Context context) {
        ...
        xToMove= (int) (100 * scale + 0.5f);
        Log.i("123","不同手机px"+xToMove);
        moveListener();
    }
                ...
                switch (MotionEventCompat.getActionMasked(motionEvent)) {
                    ...
                    case MotionEvent.ACTION_MOVE: {
                        ...
                        if (!(deltaX > 0 && !hasMoved)) {
                            if (deltaX<-200){
                                setTranslationX(frontView, -200);//防止出现一直往左滑的现象   chen
                            }else if(deltaX>0){
//                                setTranslationX(frontView, deltaX);//防止出现一直往右滑   chen
                            }else {
